...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-24 (commit b409522). Confidence: 100%.
...
### Architecture
**Files:** 131 | **Lines:** 20279 | **Import cycles:** 5
minimap-slop is a java codebase of 131 files. Ranked by PageRank over the import graph: the files most of the codebase ultimately depends on. ---
...
- `src/app` — src/app · src/app/db · src/app/project · src/app/repo
**Language:** java | **Files:** 11 | **Public symbols:** 123 / 164
Covers the 11…
- `src/app/plugin` — src/app/plugin · src/app/plugin/collection
**Language:** java | **Files:** 13 | **Public symbols:** 397 / 536
...
- `src/app/ui` — src/app/ui · src/gis/csv · src/gis/geometry · src/gis/geopackage
**Language:** java | **Files:** 13 | **Public symbols:** 162 / 204
Covers…
- `src/gis/ui` — src/gis/ui
**Language:** java | **Files:** 8 | **Public symbols:** 46 / 100
Covers the 8 source files in src/gis/ui
- `src/app/plugin/herbarium` — src/app/plugin/herbarium
**Language:** java | **Files:** 15 | **Public symbols:** 213 / 277
Covers the 15 source files in…

### Files that need care (bug-fix history first, then churn — check `get_risk` before editing)
- `src/app/plugin/collection/PrivateCollectionManager.java` — 4 commits/90d
- `src/app/ui/GUI.java` — 9 commits/90d
- `src/test/app/plugin/collection/PrivateCollectionCoreTest.java` — 4 commits/90d
- `src/app/plugin/collection/CollectionRepository.java` — 4 commits/90d
- `src/app/service/SpecimenService.java` — 6 commits/90d

### Code health
Three co-equal signals: defect risk 6.36/10 avg, hotspot health 3.4/10 (stable), worst `src/app/plugin/collection/CollectionRepository.java` at 1.0/10 · maintainability 7.15/10 · performance risk 153 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `src/app/plugin/collection/CollectionRepository.java` — change entropy — impact −3.0
- `src/app/plugin/collection/PrivateCollectionManager.java` — change entropy — impact −3.0
